...
router.post('/itemcodelist', async function (req, res) {
  var employeeitemcodes = await EmployeeModel.findOne({_id:req.body.Employee_id}).populate('DataItemCode');
  // console.log("Response",employeeitemcodes);
  var EntryDetails = [];
  for(let a  = 0 ; a < employeeitemcodes.DataItemCode.length ; a ++){
    // console.log("Testing done",employeeitemcodes.DataItemCode[a].ItemCode);
  var entry_detail = await DataItemCodeModel.findOne({ItemCode:employeeitemcodes.DataItemCode[a].ItemCode,Entry:req.body.Entry}).populate('Category Entry');
  // console.log(entry_detail);
  EntryDetails.push(entry_detail);
  if(a == employeeitemcodes.DataItemCode.length - 1){
    // console.log(employeeitemcodes.DataItemCode,req.body.Entry);
  //var EnteredData =  DataEntryModel.find({Employee_id:req.body.Employee_id,Entry:req.body.Entry});
        // await DataItemCodeModel.find({ItemCode:{$in:employeeitemcodes.DataItemCode},Entry:req.body.Entry}, async function (err, EntryDetails) {
            // console.log("test-1");
            // console.log(err);
          // if(err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
           if(EntryDetails == ""){
    ...
    return parseFloat(a.Report_SNo) - parseFloat(b.Report_SNo);
        ...
        // }).sort({createdAt:-1}).populate('Category Entry');
  }
  }
  ...
  try{
    ...
    var EntryDetails = await DataEntryModel.find({}).populate('DataItemCode Entry Authorized_To Submitted_By_Employee Department').sort({"updatedAt":-1});
     ...
     var final_data = [];
      ...
      // console.log(EntryDetails[a].auth_Submitted_Date);
...
router.post('/delete_DataEntryModel', async function (req, res) {
      await DataEntryModel.findByIdAndRemove(req.body._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
           if(user == ""){
            return res.json({Status:"Failed",Message:"No data Found", Data : {},Code:404});
           }
...
router.get('/getlist_DataItemCodeModel',async function (req, res) {
  let EntryDetails = await DataItemCodeModel.find({}).populate('Category Entry');
  res.json({Status:"Success",Message:"EntryDetails List", Data : EntryDetails ,Code:200});
});



router.get('/getlist_DataEntryModel',async function (req, res) {
  let EntryDetails = await DataEntryModel.find({});
  res.json({Status:"Success",Message:"EntryDetails List", Data : EntryDetails ,Code:200});
});
